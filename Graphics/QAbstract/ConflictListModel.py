...
filetracktypes = ['','','wffiletrack','lffiletrack','nffiletrack','uffiletrack']
...
class SyncListModel(QAbstractTableModel):
    def __init__(self, changes, newframe:Frame, maincontainer:Container, containeridtoname, option):
        ...
        headers = [FILENAME, DESCRIPTION, CURRENT, self.CURRENTREVX, self.NEWESTREVX, UPSTREAM , ACTION]
        sortdict={}
        self.changearray=[]
        ...
        if role == Qt.DisplayRole:
            if self.headers[col]==FILENAME:
                ...
                if change.wffiletrack:
                    if change.inputscenariono in [1,5] or change.reqoutscenariono in [1,5] :
                        return 'FileHeader Added'
                    if change.md5changed:
                        return 'Changed from Local Frame'
                    else:
                        return 'Same as ' + self.currentrev
                else:
                    return 'Doesnt exist or has been removed'
            elif self.headers[col]==self.CURRENTREVX:
                ...
                elif change.inputscenariono in [4,5]  or change.reqoutscenariono in [4,5] :
                    return 'Ignore Fileheader added. '
                ...
                if change.nffiletrack:
                    ...
                    if change.inputscenariono==3 or change.reqoutscenariono==3:
                        return 'Follow Upstream and Remove {}'.format(change.fileheader)
                    elif change.inputscenariono==2 or change.reqoutscenariono==2:
                ...
                return self.explainselected(row, change.fileheader, change)
    ...
    def explainselected(self, row, fileheader, change):
