...
from Graphics.Dialogs import ErrorMessage, inputFileDialog, removeFileDialog, selectFileDialog, commitDialog
import yaml
...
class UI(QMainWindow):
    ...
    def commit(self):
        ...
        if self.userdata['email'] not in self.Container.allowUsers:
            ...
            return
            #
        self.cframe, committed = self.Container.commit(self.cframe, self.commitmsgEdit.toPlainText(), self.authtoken,
                                                       BASE)
        if committed:
            self.Container.save()
